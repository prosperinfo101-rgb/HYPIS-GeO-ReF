"""
HyPIS Ug  v7.4  — Bug-Fixed + Research-Guided Fixes
═══════════════════════════════════════════════════════════════════════════════
FIXES in v7.4 (over v7.3):
  ✔ CRITICAL (Research fix): depletion_status now adds explicit "Rain event"
    skip when Pe > 0 but < ETc — irrigation is always paused on any rain day,
    even if Dr > RAW. Prevents auto-irrigating during rainfall. (Research §3)
  ✔ CRITICAL (Mbarara 0mm rain): Added detailed "why no irrigation" dry-day
    explanation panel showing Dr vs RAW comparison when IWR=0 despite no rain.
    Users can now see exactly how far the deficit is from the trigger threshold.
  ✔ Past 5 Days table: decimal places reduced (1 d.p. for mm values,
    0 d.p. for SM%, 0 d.p. for Vol) — cleaner readability.
  ✔ Rain-event status icon ☔ added to depletion_status return labels.
  ✔ Today's result section: added "Dr vs RAW gap" progress bar visual.

FIXES in v7.3 (retained):
  ✔ openpyxl graceful fallback on Python 3.14
  ✔ Styler.map (pandas 3.0), DataFrame.map
  ✔ estimate_sm cached, rh aliasing, NoneType guards, dtype safety
...
# ══════════════════════════════════════════════════════════════════════════════
# FAO-56 SOIL-TEXTURE MAD ADJUSTMENT
# ══════════════════════════════════════════════════════════════════════════════
TEXTURE_MAD_ADJ = {
    "Sand":            +0.10,
    "Loamy Sand":      +0.08,
...
.today-hdr{background:linear-gradient(90deg,#1a5fc8 0%,#2a7fd4 100%);color:#fff;
  border-radius:10px 10px 0 0;padding:8px 16px;font-weight:700;font-size:.93rem;}
.dry-no-irr{background:#fff8e8;border:1.5px solid #e8a030;border-radius:12px;
  padding:11px 16px;font-size:.87rem;margin:6px 0;}
.dry-no-irr b{color:#7a4400;}
.rain-skip{background:#e3f6fb;border:1.5px solid #29b6d6;border-radius:10px;
...
# ══════════════════════════════════════════════════════════════════════════════
# ▶▶  RESEARCH FIX v7.4  ◀◀
# depletion_status — three-tier decision:
#   1. Pe >= ETc  → rain fully covers demand (no irrigation)
#   2. Pe > 0     → rain event today — ALWAYS skip irrigation regardless of Dr
#                   (per supervisor requirement & Research §3 code patch)
#   3. Pe == 0    → dry day: check Dr vs RAW threshold
#      a. Dr ≤ 0          → field capacity
#      b. Dr ≤ raw*0.5    → adequate moisture
#      c. Dr ≤ raw        → monitor (approaching MAD, not yet triggered)
#      d. Dr ≤ taw*0.85   → irrigate (crossed MAD trigger)
#      e. Dr > taw*0.85   → urgent (near wilting point)
#
# WHY Mbarara shows 0 IWR with 0 mm rain:
#   The model is FAO-correct: irrigation only fires when Dr > RAW.
#   If initial soil moisture is high (slider ≥ 60% FC), Dr stays below RAW,
#   so the correct agronomic decision is "do not irrigate yet — soil buffer
#   is sufficient." The user now sees a detailed "Dry Day — No Irrigation"
#   explanation panel showing exactly how many mm separate Dr from the
#   trigger (RAW), so the reason is transparent.
# ══════════════════════════════════════════════════════════════════════════════
def depletion_status(dr, raw, taw, pe, etc):
    """
    FAO-56 irrigation trigger with research-guided rain-skip rule.

    Returns (label, irrigate_bool, note_str)
    """
    # ── Priority 1: rain fully covers crop demand ────────────────────────────
    if pe >= etc:
        return (
            "🌧️ Rain covers crop demand",
            False,
            f"Pe ({pe:.1f} mm) ≥ ETc ({etc:.1f} mm) — no irrigation needed today"
        )

    # ── Priority 2: ANY rain today → pause irrigation (research §3 fix) ─────
    # Even if Dr > RAW, we do not irrigate on a rain day.
    # Reason: simultaneous irrigation + rain risks waterlogging and wastes water.
    if pe > 0:
        return (
            "☔ Rain event — irrigation paused",
            False,
            f"Rain detected (Pe = {pe:.1f} mm) — irrigation skipped today. "
            f"Dr = {dr:.1f} mm vs RAW = {raw:.1f} mm. Reassess tomorrow."
        )

    # ── Priority 3: dry day — evaluate Dr vs thresholds ────────────────────
    if dr <= 0:
        return (
            "🟢 Field Capacity",
            False,
            "Soil at FC — no irrigation"
        )
    if dr <= raw * 0.5:
        return (
            "✅ Adequate moisture",
            False,
            f"Dr = {dr:.1f} mm ≤ 50% of RAW ({raw*0.5:.1f} mm) — soil buffer sufficient"
        )
    if dr <= raw:
        return (
            "🟡 Monitor — nearing MAD",
            ...
            f"gap remaining = {raw - dr:.1f} mm. Irrigate tomorrow if no rain."
        )
    if dr <= taw * 0.85:
        return (
            "⚠️ Below MAD — irrigate",
            True,
            f"Dr = {dr:.1f} mm > RAW = {raw:.1f} mm — crop stress starting"
        )
    return (
        "🔴 Near wilting point — URGENT",
        True,
...
with tab1:
    ...
    st.caption(
        ...
        f"FAO-56 PM v7.4 · Trigger: Dr ≥ RAW AND rain = 0 · "
        f"Rain event (any Pe > 0) always pauses irrigation · All values per 24-hour day"
    )
    ...
    with col1:
        ...
        st.markdown(
            ...
            f'Irrigate when Dr &gt; <b>{_raw_p:.1f} mm</b> AND rain = 0<br>'
    ...
    if st.button("🧮 Calculate Today's IWR + Past 5 Days Water History",
        ...
        with gcol2:
            ...
            if "🔴" in status_lbl:
                ...
                # ── v7.4: rain-skip panel ────────────────────────────────────
                st.markdown(
                    f'<div class="rain-skip">☔ <b>Rain event today — irrigation paused</b><br>'
                    f'Pe = {pe1:.1f} mm was received. Even though Dr = {dr_today:.1f} mm, '
                    f'irrigation is suspended on all rain days to avoid waterlogging and '
                    f'wasting water. Reassess tomorrow.</div>', unsafe_allow_html=True)
            else:
                # ── v7.4: dry-day no-irrigation explanation panel ────────────
                # This is the Mbarara case: 0mm rain but soil moisture still
                # adequate — Dr has NOT yet crossed the RAW trigger.
                gap_to_raw = max(0., raw1 - dr_today)
                pct_to_raw = min(100, int(dr_today / raw1 * 100)) if raw1 > 0 else 0
                st.markdown(
                    f'<div class="dry-no-irr">'
                    f'☀️ <b>Dry day — no irrigation needed yet</b><br>'
                    f'<b>Why no irrigation despite 0 mm rain?</b><br>'
                    f'FAO-56 rule: irrigate only when root-zone depletion (Dr) '
                    f'exceeds the Readily Available Water threshold (RAW).<br><br>'
                    f'&nbsp;&nbsp;• Dr today &nbsp;= <b>{dr_today:.1f} mm</b><br>'
                    f'&nbsp;&nbsp;• RAW trigger = <b>{raw1:.1f} mm</b><br>'
                    f'&nbsp;&nbsp;• Gap remaining = <b>{gap_to_raw:.1f} mm</b> before irrigation fires<br><br>'
                    f'The soil still holds enough water to meet crop demand without '
                    f'irrigation. If consecutive dry days continue, Dr will accumulate '
                    f'and cross RAW — at that point irrigation will be recommended.'
                    f'</div>', unsafe_allow_html=True)

                # Progress bar showing Dr as % of RAW trigger
                st.caption(f"Dr ({dr_today:.1f} mm) as % of RAW trigger ({raw1:.1f} mm):")
                st.progress(pct_to_raw / 100,
                            text=f"{pct_to_raw}% of trigger — {gap_to_raw:.1f} mm remaining")
        ...
        if hist_ctx is not None and not hist_ctx.empty:
            ...
            def _style_row(val):
                ...
                if "🌧️" in v or "☔" in v:   return "background:#e0f7fa"
            ...
            for dt, row in past_r.iterrows():
                rows_past.append({
                    ...
                    "ETc (mm/d)":      round(float(row["ETc"]),1),
                    "Pe (mm)":         round(float(row["Pe"]),1),
                    "Dr (mm)":         round(float(row["Dr_mm"]),1),   # ← 1 d.p.
                    "SM (%FC)":        int(row["SM_pct"]),              # ← 0 d.p.
                    "NIR (mm)":        round(float(row["NIR"]),1),      # ← 1 d.p.
                    "IWR gross (mm)":  round(float(row["IWR"]),1),     # ← 1 d.p.
                    "Vol (m³)":        round(float(row["Vol_m3"]),0),  # ← 0 d.p.
                    "Status":          row["Status"],
                    "Note":            row["Note"],
                })
            ...
            st.markdown(
                ...
                f'IWR = 0 unless Dr &gt; RAW and rain = 0 &nbsp;|&nbsp; '
                f'☔ = rain event → irrigation paused</small></div>',
                unsafe_allow_html=True)

            # ── v7.4: reduced decimal places in format dict ──────────────────
            _past_fmt = {
                "Rain (mm)":      "{:.1f}",
                "ETc (mm/d)":     "{:.1f}",
                "Pe (mm)":        "{:.1f}",
            ...
            styled = _styler_map(
                past_df.style.format(_past_fmt),
                _style_row, subset=["Status"])
        ...
        r4.metric("Pe (mm)",         f"{pe1:.1f}")
        r5.metric("NIR (mm/d)",      f"{nir1_day:.2f}", "ETc − Pe")
        r6.metric("Dr Deficit (mm)", f"{dr_today:.1f}", f"RAW={raw1:.1f}")
        ...
        if irrigate:
            st.markdown(
                ...
                f'NIR (net) = <b>{nir1:.1f} mm</b> · '
                f'IWR (gross, Ef={Ef:.0%}) = <b>{iwr1:.1f} mm</b><br>'
                f'Full refill gross = <b>{irr_to_fc:.1f} mm</b> = '
                f'{vol_to_fc["vol_m3"]:.1f} m³ = {vol_to_fc["vol_L"]:,.0f} L</div>',
                unsafe_allow_html=True)
            st.markdown(
                f'<div class="iwr-box">💧 <b>IWR (Gross) = {iwr1:.1f} mm/day</b> '
            ...
            st.markdown(
                ...
                unsafe_allow_html=True)
            st.warning(
                f"⚠️ **Irrigate today** — Dr = {dr_today:.1f} mm > RAW = {raw1:.1f} mm  \n"
                f"Apply **{iwr1:.1f} mm gross** = {vol1['vol_m3']:.1f} m³ "
            ...
            if pe1 >= etc1:
                st.success(
                    ...
                    f"Pe = {pe1:.1f} mm ≥ ETc = {etc1:.2f} mm/d → NIR = 0, IWR = 0.  \n"
                    f"Soil Dr = {dr_today:.1f} mm · SM = {sm_now}% of FC.")
            elif pe1 > 0:
                st.info(
                    f"☔ **Rain event today — irrigation paused.**  \n"
                    f"Pe = {pe1:.1f} mm received. Irrigation always skipped on rain days.  \n"
                    f"Dr = {dr_today:.1f} mm · RAW trigger = {raw1:.1f} mm.")
            else:
                gap = max(0., raw1 - dr_today)
                st.success(
                    f"☀️ **Dry day — soil moisture still adequate (no irrigation needed yet).**  \n"
                    f"Dr = {dr_today:.1f} mm ≤ RAW = {raw1:.1f} mm  \n"
                    f"→ Still **{gap:.1f} mm** of buffer before irrigation fires.  \n"
                    f"Crop daily deficit (NIR) = {nir1_day:.2f} mm/d — "
                    f"soil buffer covers this today. Re-check each morning.")
        ...
        today_tbl = pd.DataFrame([{
            ...
            "Pe (mm)":         round(pe1,1),
            "Dr Deficit (mm)": round(dr_today,1),
            "SM (%FC)":        sm_now,
            "NIR (mm)":        round(nir1_day,2),
            "IWR gross (mm)":  round(iwr1,1),
            "Vol needed (m³)": round(vol1["vol_m3"],1),
            "Vol (L)":         vol1["vol_L"],
            "Pump time (min)": mins1,
        ...
        if past_r is not None:
            ...
            for dt, row in past_r.iterrows():
                combined_rows.append({
                    ...
                    "ETc (mm/d)": round(float(row["ETc"]),1),
                    "Pe (mm)": round(float(row["Pe"]),1),
                    "Dr Deficit (mm)": round(float(row["Dr_mm"]),1),
                    "SM (%FC)": int(row["SM_pct"]),
                    "NIR (mm)": round(float(row["NIR"]),1),
                    "IWR gross (mm)": round(float(row["IWR"]),1),
                    "Vol (m³)": round(float(row["Vol_m3"]),1),
                    "Status": row["Status"],
                })
            combined_rows.append({
                "Date": datetime.today().strftime("%Y-%m-%d"), "Period": "TODAY",
                "Rain (mm)": round(prec_in,1), "ETc (mm/d)": round(etc1,1),
                "Pe (mm)": round(pe1,1),
                "Dr Deficit (mm)": round(dr_today,1), "SM (%FC)": sm_now,
                "NIR (mm)": round(nir1_day,1), "IWR gross (mm)": round(iwr1,1),
                "Vol (m³)": round(vol1["vol_m3"],1), "Status": status_lbl,
            })
...
with tab2:
    ...
    st.caption(
        f"FAO-56 PM · Trigger: Dr ≥ RAW AND rain = 0 · IWR = 0 on any rain day (Pe > 0) · "
    ...
    if st.button("📥 Get 5-Day Forecast", type="primary",
        ...
        if daily is None or daily.empty:
            ...
            rain_d  = (daily_r.get("precipitation", pd.Series(0,index=daily_r.index))>0).sum()

            st.info(
                f"📐 TAW = **{taw2:.1f} mm** · RAW (trigger) = **{raw2:.1f} mm** "
                f"(MAD {cp2['mad']:.2f}→{mad2:.2f})  \n"
                f"Irrigation fires only when Dr > {raw2:.1f} mm AND rain = 0 that day.")
            ...
            tb2 = daily_r[[c for c in cols_fc if c in daily_r.columns]].round(2).copy()
            tb2.rename(columns={
...
with tab3:
    ...
    st.caption(
        ...
        f"IWR=0 on any rain day (Pe>0) · NIR shown every day · "
    ...
    if st.button("📥 Retrieve Historical Data", type="primary",
        ...
        if hist is None or hist.empty:
            ...
            st.info(
                ...
                f"Irrigation fires only when Dr > {raw3:.1f} mm AND rain = 0 that day.")
            ...
            m7.metric("🌧️ Rain Days",   str((hist_r["precipitation"]>0).sum()))
...
st.caption(
    ...
    f"Trigger: Dr≥RAW AND rain=0 · IWR=0 on ANY rain day (Pe>0) · "
